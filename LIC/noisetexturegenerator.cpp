...
NoiseTextureGenerator::NoiseTextureGenerator()
    ...
    , PropPixels("pixelsDim", "Pixels dimension", vec2(512, 512), vec2(1, 1), vec2(2048, 2048), vec2(1, 1))
...
void NoiseTextureGenerator::process() {
    ...
    for (int j = 0; j < PropPixels.get().y; j++) {
        for (int i = 0; i < PropPixels.get().x; i++) {


            val = ((double)rand() / RAND_MAX) * 255 ;
            //val = (rand() & 1) ? 255 : 0 ; 
            //LogProcessorInfo("Val ->  " << val);
            ...
            //double val1 = ((double)rand() / RAND_MAX) * 100 ;
            //double val2 = ((double)rand() / RAND_MAX) * 160 ;
            //double val3 = ((double)rand() / RAND_MAX) * 255 ;
            //noiseTexture.setPixel(size2_t(i, j), vec4(val1, val2, val3, 255));
        }
    }
